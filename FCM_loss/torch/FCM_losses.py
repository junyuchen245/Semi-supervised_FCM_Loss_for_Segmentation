...
class FCM_label(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        pred = torch.reshape(y_pred, [-1, num_clus, math.prod(list(y_true.shape)[2:])])
        truth = torch.reshape(y_true, (-1, num_clus, math.prod(list(y_true.shape)[2:])))
        J_1 = 0
...
class RFCM_loss(nn.Module):
    ...
    def forward(self, y_pred, image):
        ...
        assert dim == 3 or dim == 2, 'Supports only 3D or 2D!'
        num_clus = list(y_pred.shape)[1]
        pred = torch.reshape(y_pred, (-1, num_clus, math.prod(list(y_pred.shape)[2:]))) #(bs, C, V)
        img = torch.reshape(image, (-1, math.prod(list(image.shape)[2:]))) #(bs, V)
        if dim == 3:
